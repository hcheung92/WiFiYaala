...
//sid
        sys_timeout(TCP_FAST_INTERVAL, (sys_timeout_handler)espconn_data_sentcb, psent->pespconn);
    } else {
...
//sid
		struct ip_addr *ip = (struct ip_addr *) precv->pespconn->proto.udp->remote_ip;
		if (!ip_addr_netcmp(ip, &ipconfig.ip, &ipconfig.netmask)) {

function run(color, brightness, mode, cnt)
     ...
     if mode < 5 then -- flicker
          ...
          tmr.alarm(1, time - 20, 0, function () run(color, brightness, mode, cnt) end)
     elseif mode < 8 then -- still
          local col = tmr.now() % 10
          local r,g,b = hsl2rgb(color + (col - 5), 100, brightness)
          led.set(0, r, g, b, 500)
          tmr.alarm(1, 5000, 0, function() run(color, brightness) end)
     else -- flicker
          local col = tmr.now() % 20
          local bright = tmr.now() % 50
          local r,g,b = hsl2rgb((color + (col - 10)) % 360, 100, brightness - bright)
          led.set(0, r, g, b, 300)
          tmr.alarm(1, 1000, 0, function () run(color, brightness) end)
     end
end
